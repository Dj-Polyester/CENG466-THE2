...
def center_indices(shape):
	...
	return Q/2, P/2, x, y
def dist2line(shape, angle):# linear pts have the same distance
	Q2, P2, x, y = center_indices(shape)
	m = np.tan(angle)
	return (-m*x+y+m*Q2-P2)**2/(m**2+1)
def dist2pt(shape):# circular pts have the same distance
	Q2, P2, x, y = center_indices(shape)
	return (x-Q2)**2 + (y-P2)**2
def radial_filter(f, shape, r, n):
	dist2 = dist2pt(shape)
	return f(dist2, r**2, n)

# slope
def slope(shape, angle, r): # hard slopw
	dist2 = dist2line(shape, angle)
	return dist2 < r**2

def bslope(shape, angle, r, n): # hard slope
	dist2 = dist2line(shape, angle)
	return 1/(1+(dist2/(r**2))**n)

def gslope(shape, angle, r): # Gaussian slope
	dist2 = dist2line(shape, angle)
...
def part1(input_img_path, output_path):
	...
	H = radial_filter(F.shape, GHPF, 100, 2)
